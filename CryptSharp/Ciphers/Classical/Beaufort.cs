...
{
    public class Beaufort : CipherBase<char>, IClassicalCipher
    {
        public Beaufort(char[] Alphabet) : base(Alphabet)
        {
            Key = alphabet[0].ToString();
        }
        public Beaufort(char[] Alphabet, string key, bool autoKey = false) : base(Alphabet)
        {
            Key = key;
            AutoKey = autoKey;
        }
        ...
        public string Encrypt(string clearText)
        {
            ...
            if (AutoKey)
            {
                ...
                {
                    output[i] = alphabet[(alphabet.IndexOf(Key[i % Key.Length]) - alphabet.IndexOf(output[i]) + alphabet.Length) % alphabet.Length];
                }
                for (int i = Key.Length; i < clearText.Length; i++)
                {
                    output[i] = alphabet[(alphabet.IndexOf(clearText[i - Key.Length]) - alphabet.IndexOf(output[i]) + alphabet.Length) % alphabet.Length];
                }
            }
            else
            {
        ...
        public string Decrypt(string cipherText)
        {
            ...
            if (AutoKey)
            {
                ...
                {
                    output[i] = alphabet[(alphabet.IndexOf(output[i - Key.Length]) - alphabet.IndexOf(output[i]) + alphabet.Length) % alphabet.Length];
                }
            }
            else
            {
